...
4. Check language prerequisites: Rust projects need their Rust/Cargo toolchain and rust-analyzer; TypeScript projects need their existing dependencies. TS7 uses native LSP; TS4–6 uses Node and `@vtsls/language-server` by default. Install the wrapper locally or globally, or configure `vtsls_analyzer`. To compare backends, install `typescript-language-server` and select it beside the analyzer name in the dashboard or with `bridge workspace-backend --backend typescript-language-server`. Do not silently change a project's TypeScript dependency. Explain missing prerequisites and use the user's authorized installation scope.
